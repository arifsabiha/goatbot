...
module.exports.config = {
  ...
  category: "group",
  countDown: 3,
  shortDescription: "Bot will auto-add members who leave the group",
  longDescription: "Group admin can toggle autoadd on/off with this command",
  guide: {
    en: "{pn} on/off",
    bn: "{pn} on/off"
  }
...
module.exports.onStart = async function ({ api, event, args, message, threadsData }) {
  ...
  if (!sub || !["on", "off"].includes(sub)) {
    const status = global.autoAddEnabled.get(threadID) ? "✅ Active" : "❌ Inactive";
    return message.reply(
      `🔄 AutoAdd Status: ${status}\n\n` +
      `📌 ব্যবহার:\n` +
      `• /autoadd on → চালু করো\n` +
      `• /autoadd off → বন্ধ করো\n\n` +
      `ℹ️ When enabled, bot will auto-add members who leave.`
    );
  }
