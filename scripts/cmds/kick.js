...
module.exports.onStart = async function ({ api, event, args, message }) {
  ...
  if (!adminIDs.includes(botID))
    return message.reply("Bot is not an admin in this group and cannot kick anyone.");
  ...
  if (isDisableMode) {
    ...
    await message.reply(`🔍 ${disabledIDs.length} ti disabled/Facebook User account paoa gechhe. Kicking now...`);
  ...
  for (const id of targets) {
    ...
    if (adminIDs.includes(sid)) {
      const aname = await getName(api, sid, "an admin");
      await message.reply(`🛡️ ${aname} is a group admin and cannot be kicked.`);
      continue;
    }

    const name = await getName(api, sid, "this user");

    global.recentKicks.set(`${threadID}_${sid}`, Date.now());

    try {
      await api.removeUserFromGroup(sid, threadID);
      await api.sendMessage(`👢 ${name} has been kicked from the group!`, threadID);
    } catch (kerr) {
      global.recentKicks.delete(`${threadID}_${sid}`);
      console.error("kick failed:", kerr);
      await api.sendMessage(`❌ Could not kick ${name}!\n${kerr.message || kerr.error || ""}`, threadID);
    }
