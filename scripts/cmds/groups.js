...
async function listGroups(api, message) {
        ...
        if (groups.length === 0) return message.reply("ℹ️ Bot is not in any group yet.");
...
async function showGroupInfo(api, message, threadID, mode) {
        ...
        try {
                ...
                return message.reply("❌ Could not fetch group info: " + (e?.message || e));
        }
        if (!info) return message.reply("❌ Could not find info for this thread.");
        ...
        if (mode === "admins") {
                ...
                if (adminIDs.length === 0) out += "(no admin info found)";
                return message.reply(out);
        }
        ...
        if (allIDs.length > 30) out += `  ... and ${allIDs.length - 30} more. Full list: ${global.GoatBot?.config?.prefix || "/"}groups members ${threadID}\n`;
...
module.exports.onStart = async function ({ api, message, event, args }) {
        try {
                ...
                if (sub === "info" || sub === "members" || sub === "admins") {
                        ...
                        if (!tid) return message.reply(`Please provide a Thread ID. Example: ${global.GoatBot?.config?.prefix || "/"}groups ${sub} <threadID>`);
                ...
                if (/^\d+$/.test(sub)) {
                        ...
                        const groups = list.filter(t => t && t.isGroup);
                        if (n < 1 || n > groups.length) return message.reply(`❌ Enter a group number between 1 and ${groups.length}.`);
                        return await showGroupInfo(api, message, String(groups[n - 1].threadID), null);
                }

                return message.reply(
                        `Usage:\n` +
                        `• ${global.GoatBot?.config?.prefix || "/"}groups — list all groups\n` +
                        `• ${global.GoatBot?.config?.prefix || "/"}groups <number> — details of N-th group\n` +
                        `• ${global.GoatBot?.config?.prefix || "/"}groups info <threadID> — specific group details\n` +
