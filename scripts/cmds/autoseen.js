...
module.exports.onStart = async function ({ message, args }) {
  ...
  if (sub === "on" || sub === "enable") {
    ...
    return message.reply(ok ? "✅ Auto-seen चालू করা হয়েছে (config.json এ সেভ হয়েছে)।" : "❌ Could not update settings.");
  }
  if (sub === "off" || sub === "disable") {
    const ok = setAutoSeenConfig(false);
    return message.reply(ok ? "⛔ Auto-seen disabled." : "❌ Could not update settings.");
  }
