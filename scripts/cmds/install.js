module.exports.config = {
    ...
    usePrefix: true,
    category: "Admin",
    usages: "install <name> <code_here> (or reply with code)",
    cooldowns: 5,
};
...
async function performInstall(name, sourceCode, api, threadID, messageID) {
    ...
    if (!mod.config || !mod.onStart) {
        try { fs.removeSync(filePath); } catch (e) {}
        throw new Error("❌ Invalid format! Missing config or onStart.");
    }
    ...
    return api.sendMessage(`✅ Install successful!\n📌 Name: ${name}\n⏳ Duration: 5 minutes`, threadID, messageID);
}
...
module.exports.onStart = async function ({ api, event, args }) {
    ...
    // 1. Extract code from same message if present
    if (args.length > 1) {
        sourceCode = event.body.slice(event.body.indexOf(args[1]));
    } 
    // 2. If no code in same message, check reply
    ...
    if (!name) return api.sendMessage("❌ Provide a name: /install <name>", threadID, messageID);

    if (global.GoatBot.commands.has(name) && !global.GoatBot.tempInstalls.has(name)) {
        return api.sendMessage(`⚠️ "${name}" is a permanent command. Type "delet" to replace it.`, threadID, (err, info) => {
