...
const { sleep } = require("../../utils/animation.js");
...
async function botSelfWelcome(api, threadID) {
  try {
...
module.exports.onStart = async function ({ api, event, threadsData }) {
  ...
  for (const added of addedIDs) {
    ...
    if (userID === botID) {
      await botSelfWelcome(api, threadID);
      await sleep(400);
      continue;
    }
    ...
    try {
      ...
      try {
        console.log(
          `[autonickname] failed for ${userID} in ${threadID}: ${
